...
const addTimer = (from, data) => {
    room = sesi(from, data);
    if (!room) return false;
    room.cooldown = Date.now() + toMs(40 + "s");
};
...
const pagii = (data) => {
    if (data.dead.length < 1) {
      ...
      d.length > 1
        ...
        : ""
    }\n\nTak terasa hari sudah siang, matahari tepat di atas kepala, terik panas matahari membuat suasana menjadi riuh, warga desa mempunyai 40 detik untuk berdiskusi\n*Hari ke ${
      data.day
    }*`;
        return textnya;
    }
...
async function voting(conn, x, data) {
    ...
    textnya =
        "*⌂ W E R E W O L F - G A M E*\n\nSenja telah tiba. Seluruh warga berkumpul di balai desa untuk memilih siapa yang akan dieksekusi. Sebagian warga terlihat sibuk menyiapkan alat penyiksaan untuk malam ini. Kalian mempunyai waktu selama 40 detik untuk memilih! Hati-hati, ada penghianat diantara kalian!\n\n*L I S T - P L A Y E R*:\n";
    shortPlayer(x.room, data);
    for (let i = 0; i < x.player.length; i++) {
...
async function malam(conn, x, data) {
    ...
    if (hasil_vote === 0) {
        textnya = `*⌂ W E R E W O L F - G A M E*\n\nTerlalu bimbang menentukan pilihan. Warga pun pulang ke rumah masing-masing, tidak ada yang dieksekusi hari ini. Bulan bersinar terang, malam yang mencekam telah datang. Semoga tidak ada yang mati malam ini. Pemain malam hari: kalian punya 40 detik untuk beraksi!`;
        return conn
            .sendMessage(x.room, {
                text: textnya,
                contextInfo: {
        ...
        textnya = `*⌂ W E R E W O L F - G A M E*\n\nWarga desa telah memilih, namun hasilnya seri.\n\nBintang memancarkan cahaya indah malam ini, warga desa beristirahat di kediaman masing masing. Pemain malam hari: kalian punya 40 detik untuk beraksi!`;
        let ment = [];
        return conn
            .sendMessage(x.room, {
                text: textnya,
        ...
        hasil_vote.role
      )}\n\nBulan bersinar terang malam ini, warga desa beristirahat di kediaman masing masing. Pemain malam hari: kalian punya 40 detik untuk beraksi!`;
            voteKill(x.room, data);
            let ment = [];
            ment.push(hasil_vote.id);
            return await conn
